...
module.exports = {
    ...
    "del_acc": {
        ...
        handler: (player, args) => {
            ...
            mp.players.forEach((rec) => {
                if(rec.sqlId) {
                    if(rec.account.login == args[0]) rec.kick('KICK');
                }
            });
                    ...
                    DB.Handle.query("SELECT * FROM characters WHERE accountId=?", [result[0].id], (e, result) => {
                        ...
                        result.forEach((recipient) => {
                            ...
                            DB.Handle.query("SELECT * FROM vehicles WHERE owner=?", [2000 + recipient.id], (e, result) => {
                                ...
                                if (result.length < 1) return;
                                result.forEach((recipient) => {
                                    DB.Handle.query("SELECT * FROM vehicles_mods WHERE vehicle_id=?", [recipient.id], (e, result) => {
                                        if (e) return terminal.error(e);
                                        if (result.length < 1) return;
                                        result.forEach((recipient) => {
                                            DB.Handle.query("DELETE FROM vehicles_mods WHERE vehicle_id=?", [recipient.vehicle_id]);
                                        });
                                    });
                            ...
                            DB.Handle.query("SELECT * FROM houses WHERE owner=?", [recipient.id], (e, result) => {
                                ...
                                if (result.length < 1) return;
                                result.forEach((recipient) => {
                                    var house = mp.houses.getBySqlId(recipient.id);
                                    house.setOwner(0);
                                });
                            });
                            DB.Handle.query("SELECT * FROM inventory_players WHERE playerId=?", [recipient.id], (e, result) => {
                                if (e) return terminal.error(e);
                                if (result.length < 1) return;
                                result.forEach((recipient) => {
                                    DB.Handle.query("DELETE FROM inventory_players WHERE playerId=?", [recipient.playerId]);
                                });
                            });
                            DB.Handle.query("SELECT * FROM jobs_skills WHERE playerId=?", [recipient.id], (e, result) => {
                                if (e) return terminal.error(e);
                                if (result.length < 1) return;
                                result.forEach((recipient) => {
                                    DB.Handle.query("DELETE FROM jobs_skills WHERE playerId=?", [recipient.playerId]);
                                });
                            });
    ...
    "del_char": {
        ...
        handler: (player, args) => {
            ...
            mp.players.forEach((rec) => {
                if(rec.sqlId) {
                    if(rec.name == name) rec.kick('KICK');
                }
            });
    ...
    "get_login_name": {
        ...
        handler: (player, args) => {
            ...
            DB.Handle.query("SELECT * FROM characters WHERE name=?", [name], (e, result) => {
                ...
                let rec = result[0];
                if (rec) {
                    DB.Handle.query("SELECT * FROM accounts WHERE id=?", [rec.accountId], (e, result) => {
                        terminal.info(`${player.name} посмотрел логин персонажа ${name}`);
                        terminal.log(`Login: <b>${result[0].login}</b><br/>`, player);
                        mp.logs.addLog(`${player.name} посмотрел логин персонажа ${name}`, 'main', player.account.id, player.sqlId, { level: player.admin, name: name });
                    });
                }
            });
        }
    ...
    "get_login_sc": {
        ...
        handler: (player, args) => {
            DB.Handle.query("SELECT * FROM accounts WHERE socialClub=?", [args[0]], (e, result) => {
                ...
                let rec = result[0];
                if (rec) {
                    terminal.info(`${player.name} посмотрел логин персонажа ${args[0]}`);
                    terminal.log(`Login: <b>${rec.login}</b><br/>`, player);
                    mp.logs.addLog(`${player.name} посмотрел логин персонажа ${args[0]}`, 'main', player.account.id, player.sqlId, { level: player.admin, socialClub: args[0] });
                }
            });
        }
    },
        ...
        handler: (player, args) => {
            DB.Handle.query("SELECT * FROM accounts WHERE login=?", [args[0]], (e, result) => {
                ...
                if (rec) {
                    ...
                    terminal.log(`Login: <b>${rec.login}</b><br/>SC: <b>${rec.socialClub}</b><br/>Email: <b>${rec.email} (${rec.confirmEmail === 1 ? 'Подтвержден' : 'Не подтвержден'})</b><br/>RegIP: <b>${rec.regIp}</b><br/>LastIP: <b>${rec.lastIp}</b><br/>LastDate: <b>${rec.lastDate}</b><br/>`, player);
    ...
    "info_id": {
        ...
        handler: (player, args) => {
            DB.Handle.query("SELECT * FROM logs WHERE message LIKE '%[?] авторизовался%' AND created_at < (?) ORDER BY created_at DESC LIMIT 1", [args[0], args[1] + ' ' + args[2]], (e, result) => {
                if (e) return console.log(e);
                if (result.length < 1) return terminal.error(`Игрок с ID ${args[0]} по дате: ${args[1] + ' ' + args[2]} не найден!`, player);
                let rec = result[0];
                if (rec) {
                    DB.Handle.query("SELECT * FROM characters WHERE id=?", [result[0].characterId], (e, result) => {
                        if (e) return terminal.error(e);
                        let rec = mp.players.at(args[0]);
                        if(!rec) terminal.log(`Name: <b>${result[0].name}</b><br/>Статус: <b>Оффлайн</b><br/>`, player);
                        else terminal.log(`Name: <b>${rec.name}</b><br/>Id: <b>${rec.id}</b><br/>Статус: <b>Онлайн</b><br/>`, player);
                        mp.logs.addLog(`${player.name} посмотрел ID игрока ${result[0].name} по дате ${args[1]}`, 'main', player.account.id, player.sqlId, { level: player.admin, id: args[0], date: args[1] + ' ' + args[2] });
                    });
                }
            });
        }
        ...
        handler: (player, args) => {
            DB.Handle.query("SELECT * FROM accounts WHERE login=?", [args[0]], (e, result) => {
                ...
                if (rec) {
                    ...
                    DB.Handle.query(`UPDATE accounts SET login=? WHERE login=?`, [args[1], args[0]]);
                }
            });
        }
    },
        ...
        handler: (player, args) => {
            DB.Handle.query("SELECT * FROM accounts WHERE socialClub=?", [args[0]], (e, result) => {
                ...
                if (rec) {
                    ...
                    DB.Handle.query(`UPDATE accounts SET socialClub=? WHERE socialClub=?`, [args[1], args[0]]);
                    if(recPlayer) {
    ...
    "change_pass": {
        ...
        handler: (player, args) => {
            DB.Handle.query("SELECT * FROM accounts WHERE login=?", [args[0]], (e, result) => {
                ...
                let recPlayer = mp.players.getByLogin(args[0]);
                if (rec) {
                    terminal.info(`${player.name} изменил пароль аккаунта ${args[0]} на ${args[1]}`, player);
    ...
    "change_email": {
        ...
        handler: (player, args) => {
            DB.Handle.query("SELECT * FROM accounts WHERE login=?", [args[0]], (e, result) => {
                ...
                let recPlayer = mp.players.getByLogin(args[0]);
                if (rec) {
                    terminal.info(`${player.name} изменил почту аккаунта ${args[0]} на ${args[1]}`, player);
    ...
    "set_money": {
        ...
        handler: (player, args) => {
            if (getPlayerStatus(args[1]) === "on") {
                let rec = mp.players.at(args[1]);
                if (!rec) return terminal.error(`Игрок с ID: ${args[1]} не найден!`, player);

                rec.utils.setMoney(rec.money + args[0]);
                rec.utils.success(`Администратор ${player.name} установил Вам ${args[0]}$ денег`);

                terminal.info(`${player.name} установил ${args[0]}$ денег персонажу ${rec.name}`);
                mp.logs.addLog(`${player.name} установил ${args[0]}$ денег персонажу ${rec.name}`, 'main', player.account.id, player.sqlId, { level: player.admin, money: rec.money, name: rec.name });
                mp.logs.addLog(`У аккаунта ${args[1]} установил ${player.name} - ${args[0]}$ денег`, 'main', rec.account.id, rec.sqlId, { level: player.admin, money: rec.money, name: rec.name });
            } else {
                let name = getSpecialName(args[0], player);
                if (!name) return terminal.error(`Формат: Имя_Фамилия`, player);
                DB.Handle.query("SELECT * FROM characters WHERE name=?", [name], (e, result) => {
                    ...
                    let rec = result[0];
                    let recPlayer = mp.players.getBySqlId(result[0].id);
                    if (rec) {
                        if(recPlayer) {
                            recPlayer.utils.setMoney(args[1]);
                            recPlayer.utils.success(`Администратор ${player.name} установил Вам деньги аккаунта на ${args[1]}`);
                        } else {
                            DB.Handle.query(`UPDATE characters SET money=? WHERE name=?`, [args[1], name]);
                        }

                        terminal.info(`${player.name} установил ${args[1]} денег персонажу ${name}`);
                        mp.logs.addLog(`${player.name} установил ${args[1]} денег персонажу ${name}`, 'main', player.account.id, player.sqlId, { level: player.admin, money: rec.money, name: name });
                        mp.logs.addLog(`У аккаунта ${args[0]} установил ${player.name} - ${args[1]} денег`, 'main', result[0].accountId, result[0].id, { level: player.admin, money: rec.money, name: name });
    ...
    "give_money": {
        ...
        handler: (player, args) => {
            if (getPlayerStatus(args[1]) === "on") {
                ...
                rec.utils.setMoney(rec.money + args[0]);
                rec.utils.success(`Администратор ${player.name} добавил Вам ${args[0]}$ денег`);

                terminal.info(`${player.name} добавил ${args[0]}$ денег персонажу ${rec.name}`);
                mp.logs.addLog(`${player.name} добавил ${args[0]}$ денег персонажу ${rec.name}`, 'main', player.account.id, player.sqlId, { level: player.admin, money: rec.money, name: rec.name });
                mp.logs.addLog(`У аккаунта ${args[1]} добавил ${player.name} - ${args[0]}$ денег`, 'main', rec.account.id, rec.sqlId, { level: player.admin, money: rec.money, name: rec.name });
            } else {
                ...
                DB.Handle.query("SELECT * FROM characters WHERE name=?", [name], (e, result) => {
                    ...
                    if (rec) {
                        if(recPlayer) {
                            ...
                            recPlayer.utils.success(`Администратор ${player.name} добавил Вам ${args[0]}$ денег`);
                        } else {
                            DB.Handle.query(`UPDATE characters SET money=money + ? WHERE name=?`, [args[0], name]);
                        }
                        terminal.info(`${player.name} добавил ${args[0]}$ денег персонажу ${name}`);
                        mp.logs.addLog(`${player.name} добавил ${args[0]}$ денег персонажу ${name}`, 'main', player.account.id, player.sqlId, { level: player.admin, money: rec.money, name: name });
                        mp.logs.addLog(`У аккаунта ${args[1]} добавил ${player.name} - ${args[0]}$ денег`, 'main', result[0].accountId, result[0].id, { level: player.admin, money: rec.money, name: name });
                    }
                });
            }
        }
    ...
    "take_money": {
        ...
        handler: (player, args) => {
            if (getPlayerStatus(args[1]) === "on") {
                ...
                rec.utils.success(`Администратор ${player.name} забрал у Вас ${args[0]}$ денег`);
                rec.utils.setMoney(rec.money - args[0]);
                terminal.info(`${player.name} забрал ${args[0]}$ денег у персонажа ${name}`);
                mp.logs.addLog(`${player.name} забрал ${args[0]}$ денег у персонажа ${name}`, 'main', player.account.id, player.sqlId, { level: player.admin, money: rec.money, name: rec.name });
                mp.logs.addLog(`У аккаунта ${args[1]} установил ${player.name} - ${args[0]}$ денег`, 'main', rec.account.id, rec.sqlId, { level: player.admin, money: rec.money, name: rec.name });
            } else {
                let name = getSpecialName(args[1], player);
                if (!name) return terminal.error(`Формат: Имя_Фамилия`, player);
                DB.Handle.query("SELECT * FROM characters WHERE name=?", [name], (e, result) => {
                    ...
                    if (rec) {
                        ...
                        if(recPlayer) {
                            recPlayer.utils.success(`Администратор ${player.name} забрал у Вас ${args[0]}$ денег`);
                            recPlayer.utils.setMoney(recPlayer.money - args[0]);
                        } else {
                            DB.Handle.query(`UPDATE characters SET money=money - ? WHERE name=?`, [args[0], name]);
                        }
                        ...
                        terminal.info(`${player.name} забрал ${args[0]}$ денег у персонажа ${name}`);
                        mp.logs.addLog(`${player.name} забрал ${args[0]}$ денег у персонажа ${name}`, 'main', player.account.id, player.sqlId, { level: player.admin, money: result[0].money, name: name });
                        mp.logs.addLog(`У аккаунта ${args[1]} установил ${player.name} - ${args[0]}$ денег`, 'main', result[0].accountId, result[0].id, { level: player.admin, money: result[0].money, name: name });
            ...
            DB.Handle.query("SELECT * FROM accounts WHERE login=?", [args[0]], (e, result) => {
                ...
                if (rec) {
                    ...
                    if(recPlayer) {
                        ...
                        DB.Handle.query(`UPDATE accounts SET donate=? WHERE login=?`, [args[1], args[0]]);
                    }
                    ...
                    mp.logs.addLog(`У аккаунта ${args[0]} установил ${player.name} - ${args[1]} VC`, 'main', result[0].id, 0, { level: player.admin, donate: args[1], login: args[0] });
                }
            });
        }
    },
            ...
            DB.Handle.query("SELECT * FROM accounts WHERE login=?", [args[0]], (e, result) => {
                ...
                if (rec) {
                    if(recPlayer) {
                        ...
                        DB.Handle.query(`UPDATE accounts SET donate=donate + ? WHERE login=?`, [args[1], args[0]]);
                    }
                    ...
                    mp.logs.addLog(`У аккаунта ${args[0]} добавил ${player.name} - ${args[1]} VC`, 'main', result[0].id, 0, { level: player.admin, donate: result[0].donate + args[1], login: args[0] });
        ...
        handler: (player, args) => {
            DB.Handle.query("SELECT * FROM accounts WHERE login=?", [args[0]], (e, result) => {
                ...
                if (rec) {
                    ...
                    mp.logs.addLog(`У аккаунта ${args[0]} забрал ${player.name} - ${args[1]} VC`, 'main', result[0].id, 0, { level: player.admin, donate: result[0].donate - args[1], login: args[0] });
                }
            });
        }
    },
    ...
    "mute": {
        ...
        handler: (player, args) => {
            ...
            let locktime = (args[1] * 60) * 1000; // Последний: 0 | mute
            if (locktime > 2147483647) {
              let uselogtext = `TIMEOUT ERROR | CODE: 2 | TIME: ${locktime}`;
              player.utils.error(uselogtext);
              console.log(uselogtext);
              DB.Handle.query("INSERT INTO timeout_log (log) VALUES (?)", uselogtext);
              return;
            }
            rec.muteTimerId = setTimeout(() => {
                try {
    ...
    "mute_voice": {
        ...
        handler: (player, args) => {
            ...
            let locktime = (args[1] * 60) * 1000; // Последний: 0 | mute
            if (locktime > 2147483647) {
              let uselogtext = `TIMEOUT ERROR | CODE: 3 | TIME: ${locktime}`;
              player.utils.error(uselogtext);
              console.log(uselogtext);
              DB.Handle.query("INSERT INTO timeout_log (log) VALUES (?)", uselogtext);
              return;
            }
            rec.muteVoiceTimerId = setTimeout(() => {
                try {
    ...
    "add_autosaloon_veh": {
        ...
        handler: (player, args) => {
            ...
            DB.Handle.query("INSERT INTO configvehicle (model, modelHash, brend, title, fuelTank, fuelRate, price, max) VALUES (?,?,?,?,?,?,?,?)",
    ...
    "set_bandzone_band": {
        ...
        handler: (player, args) => {
            var zone = mp.bandZones[args[0]];
            if (!zone) return mp.terminal.error(`Зона с ID: ${args[0]} не найдена!`, player);
            var band = mp.factions.getBySqlId(args[1]);
            if (!band) return terminal.error(`Организация с ID: ${args[1]} не найдена!`, player);
            if (!mp.factions.isGangFaction(band.sqlId)) return terminal.error(`${band.name} не является бандой!`, player);

            zone.setBand(band.sqlId);
            terminal.info(`${player.name} изменил банду у зоны ${zone.id} на ${band.name}`);
        }
    },
