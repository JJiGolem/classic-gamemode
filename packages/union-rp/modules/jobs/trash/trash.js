...
mp.events.add("send.trash.end", (player) => {
    try {
        if (player.job === 9) {
            ...
            mp.logs.addLog(`${player.name} заработал на работе ${money}$`, 'main', player.account.id, player.sqlId, { money: money });
            ...
            if (team.leader === player) {
                ...
                mp.logs.addLog(`${team.leader.name} заработал на работе ${nmoney}$`, 'main', team.leader.account.id, team.leader.sqlId, { money: nmoney });
