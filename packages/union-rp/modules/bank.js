const BankInfo = {
  ...
  functions: {
    ...
    putBalanceMoney(player, money) {
      ...
      mp.logs.addLog(`${player.name} положил на банковский счет ${money}$`, 'main', player.account.id, player.sqlId, { money: money });
    ...
    takeBalanceMoney(player, money) {
      ...
      player.utils.setMoney(player.money + money);
      mp.logs.addLog(`${player.name} вывел с банковского счета ${money}$`, 'main', player.account.id, player.sqlId, { money: money });
      player.utils.setBankMoney(player.bank - money);
          ...
          if (target === undefined) {
            DB.Handle.query("UPDATE characters SET bank=bank+? WHERE id=?", [money, dbplayer.id], (e, result) => {
                if (e) {
                  ...
                  mp.logs.addLog(`${name} получил на банковский счет ${money} от ${player.name}`, 'main', dbplayer[0].accountId, dbplayer[0].id, { money: money });
                });
          } else {
            target.utils.setBankMoney(target.bank + money);
            mp.logs.addLog(`${name} получил на банковский счет ${money} от ${player.name}`, 'main', target.account.id, target.sqlId, { money: money });
            target.utils.bank("Пополнение счёта", "~g~" + player.name + " ~w~перечислил на ваш счёт ~g~$" + money);
          }

          player.utils.setBankMoney(player.bank - money);
          mp.logs.addLog(`${player.name} перевел на банковскй счет ${name} - ${money}$`, 'main', player.account.id, player.sqlId, { money: money });
