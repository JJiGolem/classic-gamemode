...
mp.events.add("ls_customs::init", (player, placeId) => {
	...
	if (Array.isArray(mp.autosaloons.vehicles)) {
		...
		vehiclePrice = mp.autosaloons.vehicles.filter((v) => v.modelHash === vehicleModel).map((v) => v.price)[0] || 10000;
	}
...
// TODO (CocaColaBear): Remove later
for (const place of customsInfo.places) {
	mp.blips.new(72, place.enterPosition, {
		shortRange: true,
		scale: 0.7
	});
}
