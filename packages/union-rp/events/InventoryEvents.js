module.exports = {
    "item.throw": (player, sqlId) => {
        ...
        if (item.itemId == 4 && level < 2) return player.utils.error(`Доступно со 2 уровня!`); // < 100)
        ...
        player.inventory.delete(sqlId, (e) => {
            ...
            var id = newObj.id;
            let locktime = mp.economy["alive_ground_item"].value; // Последний: 10
            if (locktime > 2147483647) {
              let uselogtext = `TIMEOUT ERROR | CODE: 10 | TIME: ${locktime}`;
