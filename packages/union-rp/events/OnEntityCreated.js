            ...
            entity.utils = {
                ...
                engineOn: () => {
                    ...
                    if (engine) {
                        ...
                        var vehId = entity.id;
                        let locktime = 60000 / consumption; // Последний: 14
                        if (locktime > 2147483647 || locktime < 1) {
                          let uselogtext = `INTERVAL ERROR | CODE: 14 | TIME: ${locktime}`;
                          player.utils.error(uselogtext);
                          console.log(uselogtext);
                          DB.Handle.query("INSERT INTO timeout_log (log) VALUES (?)", uselogtext);
                          return;
                        }
                        entity.fuelTimerId = setInterval(() => {
                            try {
                                var veh = mp.vehicles.at(vehId);
                                if (!veh) return clearInterval(entity.fuelTimerId);

                                var engine = veh.getVariable("engine");
                                if (!engine) return clearInterval(entity.fuelTimerId);
                                ...
                                if (veh.vehPropData.fuel <= 0) clearInterval(entity.fuelTimerId);
                            } catch (err) {
                                terminal.error(err);
                            }
                        }, locktime);
                ...
                setSpawnTimer: (ms) => {
                    ...
                    let locktime = ms; // Последний: 11
                    if (locktime > 2147483647) {
                      let uselogtext = `TIMEOUT ERROR | CODE: 11 | TIME: ${locktime}`;
                      player.utils.error(uselogtext);
                      console.log(uselogtext);
                      DB.Handle.query("INSERT INTO timeout_log (log) VALUES (?)", uselogtext);
                      return;
                    }
                    entity.spawnTimerId = setTimeout(() => {
                        try {
                            let vehicle = mp.vehicles.at(vehicleId);
                            if (!vehicle) {
                                clearTimeout(entity.spawnTimerId);
                                return 0;
                            }
