module.exports = {
    "donateSystem.convertMoney": (player, donate) => {
        ...
        mp.logs.addLog(`${player.name} конвертировал ${donate * mp.economy["donate_vccoin"].value}$ валюты`, 'main', player.account.id, player.sqlId, { money: donate * mp.economy["donate_vccoin"].value, type: 'convertMoney' });
    },
    ...
    "donateSystem.buyPackage": (player, package) => {
        ...
        if(pack.type == 1) {
            ...
            mp.logs.addLog(`${player.name} приобрел Donate пакет`, 'main', player.account.id, player.sqlId, { price: pack.price, type: 'donatePackage' });
        }
    },
    ...
    "donateSystem.buyExtension": (player, extension) => {
        ...
        if(pack.sqlId < 7) {
            if(pack.sqlId == 6) {
                ...
                mp.logs.addLog(`${player.name} приобрел дополнительный слот для бизнесов`, 'main', player.account.id, player.sqlId, { price: pack.price, type: 'donateBizes' });
            } else {
                ...
                mp.logs.addLog(`${player.name} приобрел дополнительный слот для домов`, 'main', player.account.id, player.sqlId, { price: pack.price, type: 'donateHouses' });
            }
        } else {
            ...
            mp.logs.addLog(`${player.name} приобрел дополнительный слот для автомобилей`, 'main', player.account.id, player.sqlId, { price: pack.price, type: 'donateCars' });
        }
    },
