...
module.exports = {
    ...
    "authCharacter": (player, characterIndex) => {
        ...
        mp.bandZonesUtils.initBandZones(player);
        ...
        player.call("authCharacter.success", [ character.hospital > 0 ]);
        ...
        if (player.vmute > 0) {
            ...
            let locktime = (player.vmute * 60) * 1000; // Последний: 5
            if (locktime > 2147483647) {
              let uselogtext = `TIMEOUT ERROR | CODE: 5 | TIME: ${locktime}`;
              player.utils.error(uselogtext);
              console.log(uselogtext);
              DB.Handle.query("INSERT INTO timeout_log (log) VALUES (?)", uselogtext);
              return;
            }
            player.muteVoiceTimerId = setTimeout(() => {
                try {
        ...
        if (!player.phone.number) {
           ...
           DB.Handle.query(`UPDATE characters SET phone=? WHERE id=?`, [new_number, player.sqlId]);
        }
...
function initPlayerUtils(player) {
    player.utils.spawn = (health = 100, hospitalPhase = -1) => {
    ...
    player.utils.setBankMoney = (newMoney) => {
        ...
        mp.logs.addLog(`Игроку ${player.name} было назначено с ${player.bank} на ${newMoney} банковских денег`, 'main', player.account.id, player.sqlId, { oldMoney: player.bank, newMoney: newMoney });
    ...
    player.utils.setFaction = (faction, rank = 0) => {
        ...
        if (player.faction > 0) {
            ...
            if (mp.factions.isGangFaction(player.faction)) {
                mp.bandZonesUtils.initBandZones(player);
            } else player.call(`bandZones.enable`, [false]);
        }
    ...
    player.utils.setWanted = (newWanted) => {
        ...
        let locktime = mp.economy["clear_wanted_time"].value; // Последний: 16
        if (locktime > 2147483647) {
          let uselogtext = `INTERVAL ERROR | CODE: 16 | TIME: ${locktime}`;
          player.utils.error(uselogtext);
          console.log(uselogtext);
          DB.Handle.query("INSERT INTO timeout_log (log) VALUES (?)", uselogtext);
          return;
        }
        player.wantedTimer = setInterval(() => {
            try {
                var rec = mp.players.at(playerId);
                if (!rec || rec.sqlId != playerSqlId || rec.wanted <= 0) {
                    clearInterval(player.wantedTimer);
                    return 0;
                }
                rec.wanted--;
                ...
                if (rec.wanted <= 0) clearInterval(player.wantedTimer);
            } catch (err) {
    ...
    player.utils.doArrest = (cellIndex, arrestSeconds, isSpawn) => {
        ...
        player.arrestUpdaterId = setInterval(() => {
            try {
                var rec = mp.players.at(playerId);
        ...
        let locktime = arrestSeconds * 1000; // Последний: 6
        if (locktime > 2147483647) {
          let uselogtext = `TIMEOUT ERROR | CODE: 6 | TIME: ${locktime}`;
          player.utils.error(uselogtext);
          console.log(uselogtext);
          DB.Handle.query("INSERT INTO timeout_log (log) VALUES (?)", uselogtext);
          return;
        }
        player.arrestTimerId = setTimeout(() => {
            try {
                var rec = mp.players.at(playerId);
                if (!rec || rec.sqlId != playerSqlId || rec.arrestTime <= 0) {
                    clearTimeout(player.arrestTimerId);
                    return 0;
                }
    ...
    player.utils.setTie = (enable) => {
        if (enable) {
            ...
            var index = (player.sex == 1) ? 41 : 25;
            player.setClothes(7, index, 0, 0);
            player.setVariable("hasTie", true);
            player.hasTie = true;
        } else {
            delete player.hasCuffs;
            player.playAnimation("special_ped@tonya@intro", 'idle', 1, 49);
            player.utils.setLocalVar("hasCuffs", false)
            player.setClothes(7, 0, 0, 0);
            player.setVariable("hasTie", null);
...
function getPlayerSpawnPos(player) {
    ...
    if (player.demorgan > 0) {
        ...
        let locktime = (player.demorgan * 60) * 1000; // Последний: 7
        if (locktime > 2147483647) {
          let uselogtext = `TIMEOUT ERROR | CODE: 7 | TIME: ${locktime}`;
          player.utils.error(uselogtext);
          console.log(uselogtext);
          DB.Handle.query("INSERT INTO timeout_log (log) VALUES (?)", uselogtext);
        ...
        player.demorganTimerId = setTimeout(() => {
            try {
                player.utils.leaveDemorgan();
            } catch (err) {
                console.log(err.stack);
...
function initSatietyTimer(player) {
    var playerId = player.id;
    var playerSqlId = player.sqlId;
    let locktime = mp.economy["satiety_time"].value * 60 * 1000; // Последний: 15
    if (locktime > 2147483647) {
      let uselogtext = `INTERVAL ERROR | CODE: 15 | TIME: ${locktime}`;
      player.utils.error(uselogtext);
      console.log(uselogtext);
      DB.Handle.query("INSERT INTO timeout_log (log) VALUES (?)", uselogtext);
      return;
    }
    player.satietyTimer = setInterval(() => {
        try {
            var rec = mp.players.at(playerId);
            if (!rec || rec.sqlId != playerSqlId) {
                clearInterval(player.satietyTimer);
                return 0;
            }
