module.exports = {
    ...
    "sellCar": (player) => {
        ...
        if (!tempCarIds.length) return player.utils.error(`Нет доступных авто для продажи!`);
        for (var i = 0; i < tempCarIds.length; i++) {
            ...
            if (player.sqlId + 2000 != veh.owner) return player.utils.error(`Вы не владелец авто!`);
            if (dist < 15) {
                for (var sqlId in keys) {
                    mp.autosaloons.vehicles.forEach((car) => {
                        if (car.model == keys[sqlId].params.model) {
                            if (player.sqlId + 2000 != keys[sqlId].params.owner) return player.utils.error(`Вы не владелец авто!`);
                            player.call("choiceMenu.show", ["sellcar_confirm", {
                                price: car.price,
                                model: keys[sqlId].params.model
                            }]);
    
                            player.sellCarOffer = {
                                price: car.price,
                                keysSqlId: sqlId,
                                vehSqlId: keys[sqlId].params.car,
                                model: keys[sqlId].params.model,
                            };
                        }
                    });
                ...
                if(tempCarIds.length == i+1) return player.utils.error(`Нет доступных авто поблизости!`);
            }
        }
    },

    "car.sell.agree": (player) => {
        if (!player.sellCarOffer) return player.utils.error(`Предложение не найдено!`);
        
        var veh = mp.vehicles.getBySqlId(player.sellCarOffer.vehSqlId);
        var price = mp.randomInteger(30, 40);
        var count = player.sellCarOffer.price * price / 100;

        mp.fullDeleteItemsByParams(54, ["car", "owner"], [veh.sqlId, player.sqlId]);
        DB.Handle.query(`DELETE FROM vehicles WHERE id=?`, [veh.sqlId]);

        player.carIds.splice(veh.id, 1);
        player.cars.splice(veh.id, 1);
