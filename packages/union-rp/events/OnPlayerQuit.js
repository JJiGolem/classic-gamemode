...
global.savePlayerDBParams = (player) => {
    ...
    if (player.autoSaloon == true) {
		...
		const hospitalPhase = getHospitalPhase(player);

    DB.Handle.query("UPDATE characters SET minutes=minutes+?,armour=?,health=?,x=?,y=?,z=?,h=?,hospital=? WHERE id=?",
				[minutes, player.armour, player.health, pos.x, pos.y, pos.z, player.heading, hospitalPhase, player.sqlId]);
