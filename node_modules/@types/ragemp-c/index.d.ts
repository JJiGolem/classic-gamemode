...
type Timer = {id: number};


// -------------------------------------------------------------------------
// Classic RP interfaces
// -------------------------------------------------------------------------

interface TimerModule {
	init(): void;
	add(handler: (...args: any[]) => void, time: number, isInterval?: boolean, isLog?: boolean): Timer;
	remove(timer: Timer): void;
	addInterval(handler: (...args: any[]) => void, time: number, isLog?: boolean): Timer;
}

interface BusyModule {
	list: Array<string>;
	mouses: Array<string>;
	add(name: string, mouse?: boolean, nocef?: boolean): boolean;
	remove(name: string, nocef?: boolean): void;
	includes(name?: string | Array<string>): number;
}
...
interface TerminalModule {
	...
	initStorage(): void;
	initSavedCmds(): void;
	saveCmd(text: string): void;
}

interface NotifyModule {
	success(text: string, header?: string);
	warning(text: string, header?: string);
	error(text: string, header?: string);
	info(text: string, header?: string);
}
...
interface OfferDialogModule {
	show(name: string, values: object): void
	hide(): void;
}
...
interface PromptModule {
	...
	hide(): void;
	showByName(name: string): void;
}

interface UtilsModule {
	cam: CameraModule;
	getStreetName(pos: Vector3Mp): string;
	getRegionName(pos: Vector3Mp): string;
	objToString(obj: object): string;
}

interface CameraModule {
	create(x: number, y: number, z: number, toX: number, toY: number, toZ: number, fov?: number): void;
	destroy(): void;
	moveTo(x: number, y: number, z: number, toX: number, toY: number, toZ: number, time: number, fov?: number): void;
	tpTo(x: number, y: number, z: number, toX: number, toY: number, toZ: number, fov?: number): void;
}
...
interface Mp {
	...
	notify: NotifyModule;
	offerDialog: OfferDialogModule;
	chat: ChatModule;
	prompt: PromptModule;
	utils: UtilsModule;

	callCEFV(executeString: string): void;
	callCEFVN(executeObject: object): void;
	callCEFR(eventName: string, args: Array<any>): void;
	console(object: string): void;
}
