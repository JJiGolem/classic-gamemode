...
class Improvements extends Component {
    ...
    getImprovements() {
        const { business, addApp, disableHome } = this.props;
        const { activeIndex, startIndex } = this.state;

        return (
            <div className="house_improvements_list-react">
                {
                    business.improvements.map((improvement, index) => (
    ...
    getContent() {
        ...
        return (
            <Fragment>
                <div style={{ textAlign: 'center', marginTop: '26%' }}>Улучшения бизнеса</div>
...

                {
                    ...
                    <div className="house_improvements_navigate-react">
                        ...
                        <span>{ activeIndex + 1 }/{business.improvements.length}</span>
                        <button 
                            style={{ 
                                color: activeIndex == business.improvements.length - 1 && 'gray',
                                background: activeIndex == business.improvements.length - 1 && 'transparent'
                            }}
                            onClick={() => {
                                if (activeIndex < business.improvements.length - 1) {
