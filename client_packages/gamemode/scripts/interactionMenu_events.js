...
exports = (menu) => {
    ...
    mp.events.add("interactionMenu.showPlayerMenu", (player, values = null) => {
        ...
        if (gangs.includes(mp.clientStorage["faction"])) {
          if (!player.getVariable("hasTie")) {
             // переписать clear massive
             menu.execute(`interactionMenuAPI.addItemFactionInventory('default.png', 'Связать', 126, 1)`);
          } else {
             menu.execute(`interactionMenuAPI.addItemFactionInventory('default.png', 'Развязать', 0, 1)`);
             menu.execute(`interactionMenuAPI.addItemFactionInventory('default.png', 'Ограбить', 0, 0)`);
          }
        }
    });
    ...
    var playerItemHandlers = {
        ...
        "Связать": () => {
            mp.events.callRemote("use.gang.tie", interactionEntity.remoteId, false);
        },
        "Развязать": () => {
            mp.events.callRemote("use.gang.tie", interactionEntity.remoteId, true);
        },
        "Ограбить": () => {
            mp.events.callRemote("roob.gang.tie", interactionEntity.remoteId);
        },
