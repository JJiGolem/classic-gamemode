...
function onStartTuning(vehicle, isOccupant) {
	...
	mp.helpers.cameraRotator.start(camera, vehPosition, vehPosition, cameraOffset, vehHeading, 42.5);
	mp.helpers.cameraRotator.setZBound(-0.8, 1.8);
	mp.helpers.cameraRotator.setZUpMultipler(5);
...
function showConcreteMenu(modType) {
	...
	if(stockValue) {
		items.push({text: mp.helpers.string.escapeHtml(mp.game.ui.getLabelText(stockValue)), values: [getModPrice(modType, 0)]});
		itemIdx++;
	}
	...
	for (let i = 0; i < modsCount; i++) {
		...
		items.push({text: mp.helpers.string.escapeHtml(mp.game.ui.getLabelText(label)), values: [getModPrice(modType, itemIdx++)]});
	}
...
function showNeonColorMenu() {
	...
	for(let i = 0; i < neonColorNames.length; i++) {
		items.push({ text: mp.helpers.string.escapeHtml(neonColorNames[i]), values: [ getModPrice(0, i, "neonColor") ] });
	}
...
function showSpecMenu(state) {
	if (state) {
		const characteristics = mp.helpers.vehicle.getVehicleCharacteristics(mp.players.local.vehicle);
