...
function onKeyPressed(key) {
	...
	if (!mp.helpers.instructionButtonsDrawler.isActive || (!isColorShowed && !isOpacityShowed)) {
		return;
	}
...
async function initPeds() {
	...
	for (let i = 0; i < 2; i++) {
		...
		const ped = await mp.helpers.ped.createPed(model, info.spawn.position, info.spawn.heading, localPlayer.dimension);
	...
	items.map((item) => {
		...
		if (item.text === "Причёска") {
			...
			for (let i = 0; i < info.values.hair[editData.selectedGender].length; i++) {
				...
				item.values.push(mp.helpers.string.escapeHtml(label));
			}
			...
			for (let i = 0; i <= itemsCount; i++) {
				...
				item.values.push(mp.helpers.string.escapeHtml(text));
			}
...
function showButtons(state, color = false, opacity = false) {
	...
	mp.helpers.instructionButtonsDrawler.setActive(state);
}
