...
exports = (menu) => {
    ...
    mp.events.add("authCharacter.success", (inHospital = false) => {
        ...
        mainTimerId = setInterval(() => {
            ...
            if (mp.clientStorage.faction >= 9 && mp.clientStorage.faction <= 13) {
                mp.events.call("bandZones.setPlayerPos", localPlayer.position.x, localPlayer.position.y);
                if (localPlayer.getVariable("gangwar")) mp.events.call("bandZones.checkInZone", localPlayer);
            }
    ...
    mp.events.addDataHandler("passive", (entity, value) => {
        if (entity.type == "player" || entity.type == "vehicle") {
            setPassiveMode(entity, value);
        }
    });
    mp.events.addDataHandler("gangwar", gangwarDataHandler);
...
function gangwarDataHandler(entity, zoneId) {
    if (entity.type == "player" || entity.type == "vehicle") {
        if (entity.type == "player" && entity.remoteId == localPlayer.remoteId) {
            if (zoneId) {
                mp.players.forEach((rec) => {
                    if (rec.getVariable("gangwar") != zoneId) setPassiveMode(rec, true);
                    else setPassiveMode(rec, false);
                });
            } else {
                mp.players.forEach((rec) => {
                    if (rec.getVariable("gangwar")) setPassiveMode(rec, true);
                    else setPassiveMode(rec, false);
                });
            }
        } else {
            if (zoneId) {
                if (zoneId == localPlayer.getVariable("gangwar")) setPassiveMode(entity, false);
                else setPassiveMode(entity, true);
            } else {
                if (localPlayer.getVariable("gangwar")) setPassiveMode(entity, true);
