...
mp.events.add("barbershop::startBarber", async (hairColor, highlightColor, rawHeadOverlays, eyeColor) => {
	...
	mp.helpers.animation.requestAnimDict(currentPlace.animDict);
...
mp.keys.bind(Keys.E, true, () => {
	...
	mp.helpers.screen.fade(true, 50);
...
function onPedSeat() {
	...
	mp.helpers.cameraRotator.start(camera, camInfo.position, camInfo.position, camInfo.offset, camInfo.heading);
	mp.helpers.cameraRotator.setXBound(150, 240);
...
function onCutFinished() {
	...
	mp.helpers.cameraRotator.pause(false);
}
...
mp.events.add("selectMenu.itemSelected", async (menuName, itemName, itemValue, itemIndex) => {
	...
	if (menuName === "barbershop_m_main" || menuName === "barbershop_f_main") {
		...
		if (itemName === "Макияж") {
			...
			mp.helpers.instructionButtonsDrawler.setButtons(...mainInstructionButtons);
			mp.helpers.instructionButtonsDrawler.setActive(true);
			return;
		}
...
function showConcreteMenu(header) {
	...
	if (currentMenu === 0) { // Hair
		...
		mp.helpers.instructionButtonsDrawler.setButtons(...baseItemsInstructionButtons);
		mp.helpers.instructionButtonsDrawler.setActive(true);
	} else { // Other overlays
...
function generateHairValues(collection) {
	...
	for (let i = 0; i < hairValues.length; i++) {
		...
		const text = mp.helpers.string.escapeHtml(mp.game.ui.getLabelText(label));
...
function generateFacePaintingValues(collection) {
	...
	for (let i = 0; i < barberInfo.facePaintings.length; i++) {
		...
		const text = mp.helpers.string.escapeHtml(mp.game.ui.getLabelText(facePainting.l));
...
function generateEyeMakeupValues(collection) {
	...
	for (let i = 0; i < barberInfo.eyeMakeups.length; i++) {
		...
		const text = mp.helpers.string.escapeHtml(mp.game.ui.getLabelText(eyeMakeup.l));
...
function showHairInstructionButtons() {
	...
	mp.helpers.instructionButtonsDrawler.setButtons(...buttons);
	mp.helpers.instructionButtonsDrawler.setActive(true);
}
...
function playCutAnim(acceptCallback = undefined, withScissors = true) {
	...
	mp.helpers.instructionButtonsDrawler.setActive(false);
...
function onBarberStop(withAnim = true) {
	...
	mp.helpers.cameraRotator.pause(true);
	mp.helpers.cameraRotator.reset();
...
function checkPrice(itemIndex) {
	...
	return new Promise((resolve, reject) => {
		...
		mp.events.call("selectMenu.hide");
		mp.helpers.instructionButtonsDrawler.setActive(false);
		mp.helpers.loadingPrompt.show("HUD_TRANSP", 4);
